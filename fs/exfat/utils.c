...
#define C(s, v)		do { s = __exfat_checksum32(s, v); } while (0)
	...
	for (i = 0; (i + 511) < size; i += 512) {
		C_32(sum, &buf[i + 32 *  0]); C_32(sum, &buf[i + 32 *  1]);
		C_32(sum, &buf[i + 32 *  2]); C_32(sum, &buf[i + 32 *  3]);
		C_32(sum, &buf[i + 32 *  4]); C_32(sum, &buf[i + 32 *  5]);
		C_32(sum, &buf[i + 32 *  6]); C_32(sum, &buf[i + 32 *  7]);
		C_32(sum, &buf[i + 32 *  8]); C_32(sum, &buf[i + 32 *  9]);
		C_32(sum, &buf[i + 32 * 10]); C_32(sum, &buf[i + 32 * 11]);
		C_32(sum, &buf[i + 32 * 12]); C_32(sum, &buf[i + 32 * 13]);
		C_32(sum, &buf[i + 32 * 14]); C_32(sum, &buf[i + 32 * 15]);
	}
	for (; (i + 31) < size; i += 32)
		C_32(sum, &buf[i]);
	for (; i < size; i++)
...
void exfat_fs_panic(struct super_block *sb, const char *fmt, ...)
{
	...
	printk(KERN_ERR "exFAT: Filesystem panic (dev %s)\n", sb->s_id);
