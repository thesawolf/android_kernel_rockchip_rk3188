...
static int rk_fb_lcdc_state(int lcdc_id); //SAW -- void
//$_rbox_$_modify_$ end
/***************************************************************************
...
{
    	#ifdef CONFIG_LCDC_OVERLAY_ENABLE // SAW		
	struct rk_lcdc_device_driver * dev_drv = (struct rk_lcdc_device_driver * )info->par;
    	int layer_id;
//    	CHK_SUSPEND(dev_drv);
        layer_id = dev_drv->fb_get_layer(dev_drv,info->fix.id);	
...
//			if(hdmi_get_hotplug() == HDMI_HPD_ACTIVED)
			if((inf->num_fb >= 2) && rk_fb_lcdc_state(1))
				{
...
//	struct rk_fb_inf *inf = dev_get_drvdata(info->device);
// SAW -- moved into CONFIG_MALI def  
...
//$_rbox_$_modify_$ zhengyang modified for box display system
#if 1
int rk_fb_lcdc_state(int lcdc_id) //SAW -- void)
{
	struct rk_fb_inf *inf =  platform_get_drvdata(g_fb_pdev);
	struct rk_lcdc_device_driver * dev_drv = NULL;
	struct fb_info *info = NULL;
	char name[6];
	int i, layer_id; //SAW -- , active = 0;
