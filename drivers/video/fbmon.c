		...
		int ver, int rev) //SAW -- 1080p
{
	...
	switch (ratio) {
	case 0:
		/* SAW -- in EDID 1.3, 0 changed to 16:10 (prior 1:1) */
...
static int get_dst_timing(unsigned char *block,
			  struct fb_videomode *mode, int ver, int rev) //SAW
{
	int j, num = 0;

	for (j = 0; j < 6; j++, block += STD_TIMING_DESCRIPTION_SIZE)
		num += get_std_timing(block, &mode[num], ver, rev); //SAW
...
static struct fb_videomode *fb_create_modedb(unsigned char *edid, int *dbsize)
{
	...
	ver = edid[EDID_STRUCT_VERSION]; //SAW
	rev = edid[EDID_STRUCT_REVISION]; //SAW
	...
	*dbsize = 0;
	...
	for (i = 0; i < 4; i++, block+= DETAILED_TIMING_DESCRIPTION_SIZE) {
		if (block[0] == 0x00 && block[1] == 0x00 && block[3] == 0xfa)
			num += get_dst_timing(block + 5, &mode[num], ver, rev); //SAW
	}
